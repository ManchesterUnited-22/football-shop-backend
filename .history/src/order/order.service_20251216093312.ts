...
export class OrderService {
    ...
    async findOneWithDetails(id: number): Promise<Order & { items: any[] } | null> {
        // Sử dụng Prisma để query đơn hàng và include Order Items.
        // Bạn có thể cần điều chỉnh phần include này dựa trên Prisma Schema thực tế của bạn
        // để đảm bảo nó lấy đủ thông tin cần thiết cho ShippingService (ví dụ: product name, quantity).
        return this.prisma.order.findUnique({
            where: { id },
            include: { 
                items: true // Phải là tên quan hệ trong Prisma Schema của bạn
            }, 
        });
    }
