...
export class OrderController {
  ...
  // @UseGuards(JwtAuthGuard) // Giả sử có bảo vệ
  async confirmDelivery(
    @Param('id') id: string,
    // @Req() req: any // Giả sử lấy userId từ request nếu có bảo vệ
  ) {
    // Trong thực tế, bạn cần xác minh rằng req.user.id sở hữu đơn hàng này
    // const userId = req.user.id;
    // return this.orderService.updateStatusForUser(+id, userId, 'DELIVERED');
    
    // Tạm thời, chúng ta gọi hàm chuyển trạng thái chung
    return this.orderService.updateStatus(+id, 'DELIVERED'); 
  }
}
