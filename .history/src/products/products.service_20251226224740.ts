...
// ✅ INTERFACE MỚI: Dùng cho báo cáo tồn kho thấp
...
        console.log(`[ACCESSORY]: Tạo biến thể mặc định cho sản phẩm Phụ kiện (ID: ${categoryId}).`);
        variantData = [{
            sizeValue: 'ONESIZE', // Kích thước mặc định
            stock: 9999, // Tồn kho cao, cho phép mua hàng
            color: 'Default',
...
    // ===================================================
    // CẬP NHẬT TỒN KHO CHO PRODUCT VARIANT (Quick Restock)
    // ===================================================
...
        }
        
        // 2. Kiểm tra tồn kho mới phải là số dương hoặc 0
        if (newStock < 0) {
            throw new BadRequestException('Số lượng tồn kho mới không thể là số âm.');
        }
...
    }
    ...
    // ===================================================
    // ✅ BÁO CÁO TỒN KHO THẤP (Low Stock Report)
    // ===================================================
    async getLowStockItems({ threshold, categoryId, limit }: LowStockFilter) {
        ...
        const where: Prisma.ProductVariantWhereInput = {
            stock: {
                lte: threshold, // Lọc theo Ngưỡng Tồn Kho
            },
        };

        // 2. Thêm điều kiện lọc Category nếu có
        if (categoryId) {
            where.product = {
        ...
        const lowStockItems = await this.prisma.productVariant.findMany({
            ...
            orderBy: {
                stock: 'asc', // Sắp xếp từ thấp đến cao
            },
