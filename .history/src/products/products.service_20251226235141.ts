...
export class ProductsService {
  ...
  private applyPromotion(price: number, discount: number | null, start: Date | null, end: Date | null) {
    const now = new Date();
    // Điều kiện Sale: Có % giảm > 0 AND nằm trong thời hạn Start - End
    const isSale = 
      (discount ?? 0) > 0 && 
      start !== null && 
      end !== null && 
  ...
  // =========================================
  // 3. CHI TIẾT SẢN PHẨM (XỬ LÝ GIÁ KÉP)
  // =========================================
  async findOne(slug: string) {
    ...
    const modifiedVariants = variants.map(v => {
      ...
      // 2. Áp dụng giảm giá lên giá size đó
