...
    // ⭐️ FIX: TẠO SLUG SẠCH TRƯỚC KHI GỌI PRISMA CREATE ⭐️
    
    // 1. Làm sạch tên sản phẩm: Loại bỏ khoảng trắng và ký tự không an toàn (ví dụ: '/')
    const cleanedName = name
        .toLowerCase()
        .replace(/\s+/g, '-')  // Thay thế khoảng trắng (một hoặc nhiều) bằng gạch ngang
        .replace(/[^\w-]+/g, ''); // Loại bỏ tất cả ký tự không phải chữ, số, hoặc gạch ngang (FIX LỖI 404 FRONTEND)

    // 2. Tạo chuỗi slug cuối cùng (đảm bảo tính duy nhất)
...
        sizeValue: size,
        stock: 0, 
        // Sku sử dụng name gốc, không dùng cleanedName
...
            data: {
                name,
                slug: productSlug, // ⭐️ SỬ DỤNG SLUG ĐÃ TẠO SẠCH ⭐️
                description,
                price: price, 
                images: images || [], 
