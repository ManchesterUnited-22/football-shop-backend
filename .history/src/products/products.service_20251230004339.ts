...
async update(id: number, updateData: any) {
  ...
  if (incomingVariants) {
    ...
    incomingVariants.forEach((v: any) => {
      // CHUẨN HÓA DỮ LIỆU VARIANT (Chỉ lấy đúng các trường Prisma cần)
      ...
      if (v.id && existingIds.includes(v.id)) {
        ...
        transactionOps.push(this.prisma.productVariant.update({
          ...
          data: variantPayload, // <--- Dùng payload đã lọc, không dùng ...v
        }));
      } else if (!v.id || v.id.toString().startsWith('temp-')) {
        // TẠO BIẾN THỂ MỚI
        transactionOps.push(this.prisma.productVariant.create({
          data: {
            ...variantPayload, // <--- Dùng payload đã lọc
            productId: id
          },
        }));
      }
