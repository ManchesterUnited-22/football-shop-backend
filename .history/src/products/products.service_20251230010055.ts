...
async update(id: number, updateData: any) {
  // 1. Tách mảng variants và các trường quan hệ/hệ thống ra
  const { 
    variants: incomingVariants, 
    id: _id, 
    categoryId, 
  ...
  // 2. Chuẩn bị dữ liệu cho Sản phẩm (Product)
  const productFields: any = { ...restFields };
  ...
  // QUAN TRỌNG: Sửa lỗi "Unknown argument categoryId"
  // Thay vì để categoryId ở ngoài, ta dùng 'connect'
  const targetCategoryId = categoryId || (category && category.id);
  if (targetCategoryId) {
    productFields.category = {
      connect: { id: Number(targetCategoryId) }
    };
  }
  ...
  // 4. Cập nhật Sản phẩm cha - Đã loại bỏ categoryId thô
  ...
  return this.prisma.$transaction(transactionOps).then(results => results[results.length - 1]);
}
