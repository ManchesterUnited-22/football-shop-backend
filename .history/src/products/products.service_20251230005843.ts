...
async update(id: number, updateData: any) {
    // 1. Tách variants và loại bỏ các trường không được phép cập nhật trực tiếp (như id)
    const { variants: incomingVariants, id: _id, createdAt, updatedAt, ...restFields } = updateData;
    
    const productFields = { ...restFields };

    // 2. Ép kiểu dữ liệu an toàn cho Product
    ...
    if (incomingVariants) {
      ...
      const keepIds = incomingVariants.filter((v: any) => v.id).map((v: any) => Number(v.id));
      const toDelete = existingIds.filter(eid => !keepIds.includes(eid));

      // Xóa các variant cũ không còn trong danh sách gửi lên
      if (toDelete.length > 0) {
        transactionOps.push(this.prisma.productVariant.deleteMany({ where: { id: { in: toDelete } } }));
      }

      incomingVariants.forEach((v: any) => {
        // CHỈ LẤY đúng các trường của ProductVariant, không lấy dư
        ...
        if (v.id && existingIds.includes(Number(v.id))) {
          ...
          // CREATE variant mới (Tuyệt đối không dùng ...v)
    ...
    console.error("LỖI PRISMA CỤ THỂ:", error); // Log này sẽ hiện ở Terminal Backend của bạn
    throw error;
  }
  }
