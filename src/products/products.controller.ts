...
export class ProductsController {
  ...
  // Thêm hàm này để lấy sản phẩm theo ID cho Admin
@Get('id/:id')
@UseGuards(AuthGuard, RolesGuard)
@Roles(Role.ADMIN)
findOneById(@Param('id', ParseIntPipe) id: number) {
  return this.productsService.findById(id); // Đảm bảo trong Service bạn đã có hàm findById
}
  ...
  @Post('global-promotion') 
  @UseGuards(AuthGuard, RolesGuard)
  @Roles(Role.ADMIN)
  async handleGlobalSale(@Body() body: any) {
    // Gọi đúng hàm applyGlobalPromotion trong Service đã viết
    return this.productsService.applyGlobalPromotion(body);
  }
}
